...
Next, we explored linear models and tried to use them to predict the likelihood of an employee leaving the company.
...
Next we have been using Support Vector Machines (SVM) to predict the employee turnover.
...
In this chapter, we explored Artificial Neural Networks (ANN). ANN is a machine learning algorithm inspired by the biological structure of the human brain. We will explain how ANN work, how to train an ANN model, and how to use it to predict employee turnover.
...
It has been interesting for our group to try those concepts in a practical approach, once we have learned them in class. Now we understand them not only in theory, but also in an applied way. Thank you for answering all our questions in classes, we highly appreciated!
